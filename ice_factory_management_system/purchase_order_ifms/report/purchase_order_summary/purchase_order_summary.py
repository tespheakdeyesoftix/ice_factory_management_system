...
def get_columns(filters):
	columns = []
	if filters.row_group == 'Purcahse Order Invoice':
...
def get_report_data(filters,parent_row_group=None,indent=0,group_filter=None):
	...
	if(filters.row_group == "Sale Invoice"):
		sql = "select {0} as row_group, {1} as indent ".format(row_group, indent)
	else:
	...
	if (indent == 1 and filters.parent_row_group) or (indent == 0 and (filters.parent_row_group or "") == ""):
		...
		if filters.row_group == "Product And Price":
			extra_columns +=  ",a.price,a.unit"
			extra_columns_group_by += ",a.price,a.unit"
	...
	# frappe.throw(str({"sql": sql, "filter": filters}))
...
def get_conditions(filters,group_filter=None):
	conditions = " 1 = 1 and b.docstatus = 1 "
	...
	if filters.party_type:
		conditions += " AND b.party_type = %(party_type)s"

	if filters.party:
		conditions += " AND b.party  = %(party)s"
...
def get_report_field(filters):
	...
	fields.append({"label":_("Cost"), "short_label":"Cost", "fieldname":"cost","fieldtype":"Currency","indicator":"Blue","precision":None, "align":"right","chart_color":"#2E7D32","sql_expression":"sum(a.cost)"})
	fields.append({"label":_("Amount"), "short_label":"Amount", "fieldname":"amount","fieldtype":"Currency","indicator":"Blue","precision":None, "align":"right","chart_color":"#2E7D32","sql_expression":"sum(a.total_cost)"})
...
def get_row_groups():
	return [
		...
		{
			"fieldname":"if(ifnull(b.party_type,'')='','Not Set',b.party_type)",
			"label":"Party Type",
			"parent_row_group_filter_field":"row_group"
		},
		{
